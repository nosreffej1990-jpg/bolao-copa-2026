...
- **Correção dos Próximos Confrontos na Tela Inicial**: Atualizamos a função `fetchLiveMatches` no arquivo [page.js](file:///C:/Users/nosre/OneDrive/Documentos/GitHub/bolao-copa-2026/src/app/page.js) para buscar e formatar os próximos confrontos utilizando a função centralizada `fetchAllGames`. Isso garante a tradução correta dos nomes dos classificados (ex: "South Africa" -> "África do Sul"), a exibição das bandeiras correspondentes, o fuso horário de Brasília e o fallback para os rótulos de definição (como "Runner-up Group A") caso o confronto ainda não esteja definido.
- **Bloqueio Automático de Aposta por PIX (Mata-Mata)**: Reestruturamos a segurança do backend para que um jogador comum (`role === 'Jogador'`) só consiga enviar palpites caso esteja explicitamente liberado para a fase correspondente (através das colunas `approved_r32`, `approved_r16`, etc. do banco). No exato instante em que o palpite é salvo com sucesso, o backend revoga automaticamente a sua autorização (definindo o campo correspondente como `false`), bloqueando a tela de apostas imediatamente. Para realizar um novo envio, o jogador deverá efetuar um novo pagamento e o administrador (Junior) deverá liberá-lo no painel de controle.
- **Liberação Automática para Novos Jogadores (Fase de Grupos Concluída)**: Agora que a fase de grupos acabou, novos cadastros de jogadores são automaticamente marcados como `approved: true` no banco de dados e no cliente. Isso elimina a barreira do modal inicial de pagamento/liberação da Fase de Grupos, permitindo acesso direto ao dashboard após o registro. Os bloqueios individuais por faturamento agora ocorrem somente dentro da aba do Mata-Mata.
- **Gerenciador de Usuários (Edição e Exclusão)**: Adicionamos suporte total para gerenciamento administrativo dos usuários a partir do painel ("Gerenciador de Jogadores"). Agora o administrador pode excluir jogadores do banco de dados (excluindo os botões "Excluir") ou editar seus dados de perfil (Nome de Usuário, WhatsApp, Perfil/Função de Acesso, Senha) diretamente através de um modal de edição integrado.
- **Correção Definitiva de Horário na Tela Inicial**: Substituímos o carregamento da API de próximos jogos na tela de login por uma consulta direta aos dados da tabela `confrontos` sincronizada no banco de dados/localStorage. Isso garante que a página inicial mostre exatamente as mesmas datas, horários e traduções corretas do fuso de Brasília que o jogador vê dentro do dashboard.
- **Acesso do Moderador (Junior) ao Mata-Mata**: Ajustamos as verificações de liberação na tela de apostas do Mata-Mata para permitir que tanto a role `Admin` quanto a role `Moderador` (caso do usuário Junior) consigam acessar e salvar palpites do chaveamento sem serem afetados pelas travas automáticas do PIX que afetam os jogadores.
